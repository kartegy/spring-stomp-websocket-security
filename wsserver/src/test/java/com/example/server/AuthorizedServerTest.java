...
public class AuthorizedServerTest {
    ...
    @Test
    public void shouldSendBackAnEchoToAuthorizedUser() throws Exception {
    ...
    @Test
    @DirtiesContext
    public void shouldNotSendBackAnEchoToNonAuthorizedUser() throws Exception {
            ...
            public void afterConnected(final StompSession session, StompHeaders connectedHeaders) {
                ...
                session.subscribe("/user/topic/greetings", new StompFrameHandler() {
                    ...
                    public void handleFrame(StompHeaders headers, Object payload) {
                        MyMessage response = (MyMessage) payload;
                        try {
                            assertEquals("Server response: ", response.getContent());
                        } catch (Throwable t) {
                            failure.set(t);
                        } finally {
                            session.disconnect();
        ...
        // Basic HTTP authentication
        String userData = "aNotAuthUser" + ":" + "secret";
        String auth = "Basic " + Base64.getEncoder().encodeToString(userData.getBytes());
        this.headers.add(HttpHeaders.AUTHORIZATION, auth);
